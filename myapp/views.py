...
def detail(req,pk):
    ...
    try:
        df=pd.read_csv(file_value.upload_dir.path,index_col=0,skip_blank_lines=True,skiprows=0)
    except UnicodeDecodeError:
        df=pd.read_csv(file_value.upload_dir.path,index_col=0,skip_blank_lines=True,encoding='cp932',skiprows=0)
    '''
    trc=[]
    flag=0
    for i in range(len(df.index)):
        for j in range(len(df.columns)):
            if df.iat[i,j]=="Test Item":
                flag=1
        trc.append(i)
        if flag==1:
            break
    df.drop(df.index[trc],inplace=True)
    '''
    context={
...
def inputdata(pk):
    file_value=get_object_or_404(FileUpLoad,id=pk)
    try:
        df=pd.read_csv(file_value.upload_dir.path,skip_blank_lines=True,index_col=0,skiprows=0) 
    except UnicodeDecodeError:
        df=pd.read_csv(file_value.upload_dir.path,skip_blank_lines=True,index_col=0,encoding='cp932',skiprows=0)
    #df.drop(df.index[[1,2,3,4,5,6,7,8]])     #データフレームから余分な行を削除
    TestName="spades"
    df.replace(' ','',regex=True,inplace=True)
    df.set_index('suit',inplace=True)
    Test = df.at[TestName,'value']
    n = len(Test)
